...
const Button = forwardRef(({ className, variant, size, asChild = false, cursorTarget = true, ...props }, ref) => {
  const Comp = asChild ? Slot : 'button'
  return (
    <Comp
      ref={ref}
      className={cn(buttonVariants({ variant, size }), cursorTarget && 'cursor-target', className)}
      {...props}
    />
  )
})
